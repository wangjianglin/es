...
class PtrUIHandlerHolder implements PtrUIHandler {
    ...
    public void onUIPositionChange(PtrView ptr, boolean isUnderTouch, PtrView.Status status, PtrIndicator ptrIndicator) {
        ...
        do {
            final PtrUIHandler handler = current.getHandler();
            if (null != handler) {
                handler.onUIPositionChange(ptr, isUnderTouch, status, ptrIndicator);
            }
        } while ((current = current.mNext) != null);
    }
}
